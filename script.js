...
function checkBookingTime() {
    ...
    if (time === "") {
        message.textContent = "Please choose a booking time first.";
    } else if (time >= "12:00" && time <= "22:00") {
        message.textContent = "Great choice! Habibi Bites is open for bookings at this time.";
    } else {
        message.textContent = "Sorry, bookings are only available from 12:00 PM to 10:00 PM.";
    }
}

const bookingForm = document.getElementById("cs-form-490");
if (bookingForm) {
    bookingForm.addEventListener("submit", function(event) {
        event.preventDefault();
        alert("Thank you! This demo booking request has been received.");
    });
}
